...
import {extract_variables_from_aggregate,extract_variable_from_global_expression,extract_global_pos_neg_variables,extractPositiveVariables } from "./utility_parsing";
...
export function checkIsRule(constructs: [string, number, number][]) {
	...
	for (let i = 0; i < constructs.length; i++) {
		...
		if (constructs[i][1] === ASPCore2Lexer.CONS && head) { 
			return true;
		}
	}
	return false;
...
*/
export function checkSafe(global_head: string[], global_body: string[], global_negative: string[],global_expression: string[],global_equal: string[],local_head_aggregate: string[],local_negative: string[],local_positive: string[],local_expressions:string[],local_equals:string[]) {
	let safe = true;
	const failedConditions: string[] = [];
	const global_expr: string[] = [];
...
*/
export function tokenize_head_body(constructs: [string, number, number][], atoms: string[], lineOfText:vscode.TextLine) {
    ...
    for (let i = 0; i < constructs.length; i++) {
		...
		if (constructs[i][1] === ASPCore2Lexer.CONS && !skip) { 
            head = !head;
        }
		...
		if(s===ASPCore2Lexer.EQUAL){
			equal=true;
		}
		if (constructs[i][1] === ASPCore2Lexer.SYMBOLIC_CONSTANT && !skip) { 
